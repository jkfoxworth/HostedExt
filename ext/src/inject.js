...
function fetchResultData(callback) {
  var searchResults = $('button#total span.count').text();
  if (searchResults) {
    searchResults = parseInt(searchResults);
  } else {
    searchResults = 0;
  }
...
function nextPage(sendResponse) {
  $("#top-bar span.page-count  a[title*='Next page'] > li-icon").click();
  setTimeout(function() {
    var x = document.readyState;
    if (x !== "complete") {
      awaitNextPage(sendResponse);
    } else {
      sendResponse('done');
    }
  }, 3000);
...
function awaitNextPage(sendResponse) {
  setTimeout(function() {
    var x = document.readyState;
    if (x !== "complete") {
      awaitNextPage(sendResponse);
